...
| `save-inspection-item-state` | POST | Phase 1C. Flips a single `inspection_items` row's `gathered_at` / `done_at` (with optional `done_by`). Used by Overview + Worklist checkbox toggles. **May 2026:** when a `done_at` toggle completes the last open flagged **work-type** item on the parent inspection, sends "Turnover Task Status: Complete — ready for cleaning" email — gather/purchase items don't gate the trigger because they're shopping that can lag the on-site work. Idempotent via conditional `WHERE tasks_complete_email_sent_at IS NULL` update so concurrent toggles can't double-fire. Email body also reports any still-pending Gather items so the team sees what's left. |
...
| Last open flagged **work** item gets checked Done in Overview (Gather items don't gate this) | `Turnover work complete — ready for cleaning: <address>` | `Turnover Task Status: Complete` | `save-inspection-item-state.js` |
...
**"Turnover work complete — ready for cleaning" email didn't fire when I checked the last box** — Two common causes: (1) The last box you checked was a Gather item, not a Task. The trigger only counts `item_type='work'` rows because Gather is shopping that can lag the work — checking off bulbs you bought doesn't make the property ready to clean. Check the Tasks section in the Overview to see if any work items are still pending. (2) `inspections.tasks_complete_email_sent_at` is already set. The idempotency guard skipped — un-check + re-check loops won't re-trigger. The marker is cleared automatically when the inspector saves with new flagged items; if you need to force a re-send manually, set the column back to NULL in Supabase.
