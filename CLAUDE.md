...
| `save-inspection-item-state` | POST | Phase 1C. Flips a single `inspection_items` row's `gathered_at` / `done_at` (with optional `done_by`). Used by Overview + Worklist checkbox toggles. **May 2026:** when a `done_at` toggle completes the last open flagged item on the parent inspection (Gather + Tasks both gate it — the property is only "ready for cleaning" once shopping is in hand too), sends the "Turnover Task Status: Complete — ready for cleaning" email. Idempotent via conditional `WHERE tasks_complete_email_sent_at IS NULL` update so concurrent toggles can't double-fire. Email check is awaited (not fire-and-forget) so the Brevo POST completes before Netlify can freeze the container. |
...
| Last open flagged item (Gather + Tasks both gate) gets checked Done in Overview | `Turnover complete — ready for cleaning: <address>` | `Turnover Task Status: Complete` | `save-inspection-item-state.js` |
...
**For UI-only changes, prefer `vite` over `netlify dev`.** `.claude/launch.json` has a `vite` entry (`npm run dev`, port 5173) that reads from the worktree CWD — no main-worktree dance, much faster startup. Use it whenever your change doesn't touch `netlify/functions/` or rely on `/api/*` endpoints. Only fall back to the `mills-dashboard` (netlify-cli) entry when you need the functions to run.
...
**"Turnover complete — ready for cleaning" email didn't fire when I checked the last box** — Three common causes: (1) Some other flagged row is still pending. Both Gather and Tasks gate this email; check the Overview for any unchecked items. (2) `inspections.tasks_complete_email_sent_at` is already set. The idempotency guard skipped — un-check + re-check loops won't re-trigger. The marker is cleared automatically when the inspector saves with new flagged items; if you need to force a re-send manually, set the column back to NULL in Supabase. (3) Pre-May-2026 deploys had a fire-and-forget bug where the Brevo POST got cut off when Netlify froze the function container — current code awaits the email before returning, so this shouldn't happen on the deployed version.
